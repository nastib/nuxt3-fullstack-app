          ...
          <div class="rounded-md shadow-sm -space-y-px mb-1">
            <div>
              ...
              <input
                ...
                placeholder="Name"
              />
              <span class="text-xs" :class="errors.has('name') ? 'text-red-500' : ''"> {{ errors.get('name')?.check?.errorMessage }}</span>
            </div>
          </div>
          <div class="rounded-md shadow-sm -space-y-px mb-1">
            <div>
              ...
              />
              <span class="text-xs" :class="errors.has('username') ? 'text-red-500' : ''">
                {{ errors.get('username')?.check?.errorMessage }}</span
              >
            </div>
          </div>
          <div class="rounded-md shadow-sm -space-y-px mb-1">
            <div>
              ...
              />
              <span class="text-xs" :class="errors.has('email') ? 'text-red-500' : ''"> {{ errors.get('email')?.check?.errorMessage }}</span>
            </div>
          </div>
          <div class="rounded-md shadow-sm -space-y-px mb-1">
            <div>
              ...
              />
              <span class="text-xs" :class="errors.has('password') ? 'text-red-500' : ''">
                {{ errors.get('password')?.check?.errorMessage }}</span
              >
            </div>
          </div>
...
import { userRegister } from '@/composables/useAuth';
...
const postRegisterForm = async () => {
  response.value = await userRegister(username.value, name.value, email.value, password.value);
  //@ts-ignore

...
<template>
  <div class="h-screen  dark:from-slate-800 dark:to-slate-400">
    <div class="flex items-center justify-center  px-4 sm:px-6 lg:px-8">
      <div class="max-w-md w-full space-y-8">
        ...
        <div v-if="response.hasErrors && errors" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
          role="alert">
        ...
        <form v-if="true" v-on:submit.prevent class="mt-8 space-y-6" action="#" method="POST">
          ...
          <div>
            ...
            <input v-model="password" id="password" name="password" type="password" autocomplete="current-password"
              ...
              <span class="text-xs" :class="errors.has('password') ? 'text-red-500' : ''"> {{ errors.get('password')?.check?.errorMessage }}</span>
          </div>
...
const postLoginForm = async function () {
 ...
 response.value =  await userLogin(email.value, password.value)
   //@ts-ignore
